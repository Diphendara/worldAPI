...
class CountryController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $countries = $em->getRepository('worldAPIBundle:Country')->findAll();
    ...
    {
        $country = new Country();
        $form = $this->createForm('worldAPIBundle\Form\CountryType', $country);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('country_show', array('code' => $country->getCode()));
        }
     ...
     *
     * @Route("/{code}/edit", name="country_edit")
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Country $country)
    {
        $deleteForm = $this->createDeleteForm($country);
        $editForm = $this->createForm('worldAPIBundle\Form\CountryType', $country);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('country_edit', array('code' => $country->getCode()));
        }
     ...
     *
     * @Route("/{code}", name="country_delete")
     * @Method("DELETE")
     */
    ...
    private function createDeleteForm(Country $country)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('country_delete', array('code' => $country->getCode())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
